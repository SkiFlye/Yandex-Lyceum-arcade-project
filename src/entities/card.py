...
class Card:
    """Класс карты"""
    suit: str  # 'sword', 'shield', 'potion'
    value: int
    actual_card_width: int = 0
    actual_card_height: int = 0

    def __post_init__(self):
        # Используем правильный путь к изображению карты
    ...
    def get_effect_power(self, player_level=1, base_damage_per_level=0.2, base_block_per_level=0.15):
        # Для карт зелья эффект не зависит от уровня игрока
        if self.suit == 'potion':
            # Базовая сила зелья пропорциональна его уровню
            return self.value  # Возвращаем уровень зелья для определения эффекта
